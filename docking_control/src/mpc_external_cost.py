...
class MPC:
    def __init__(self, auv, mpc_params):
        ...
        self.R_delta = np.diag(mpc_params["penalty"]["R_delta"])
    ...
    def reset(self):
        self.previous_control = np.zeros((self.control_axes, 1))
        self.previous_state = np.zeros((12, 1))
    ...
    def auv_model(self):
        ...
        u = SX.sym("u", (self.control_axes, 1))
        p = SX.sym("p", (2 * self.vehicle_state_dim + self.control_axes, 1))
        residual = p[self.vehicle_state_dim : 2 * self.vehicle_state_dim]
    ...
    def create_ocp_solver_description(self):
        ...
        self.acados_ocp.parameter_values = np.zeros(
            2 * self.vehicle_state_dim + self.control_axes
        )

        xr = self.acados_model.p[0 : self.vehicle_state_dim]
        residual = self.acados_model.p[
            self.vehicle_state_dim : 2 * self.vehicle_state_dim
        ]
        u_prev = self.acados_model.p[2 * self.vehicle_state_dim :]
        ...
        # self.acados_ocp.cost.W = block_diag(self.Q, self.R, self.R_delta)
        # self.acados_ocp.model.cost_y_expr = vertcat(x, u, u - u_prev)
        # self.acados_ocp.cost.yref = np.zeros(nx + nu + nu)
        ...
        self.acados_model.cost_expr_ext_cost = (
            error_x.T @ self.Q @ error_x
            + u.T @ self.R @ u
            + delta_u.T @ self.R_delta @ delta_u
        )
        self.acados_model.cost_expr_ext_cost_e = error_x.T @ self.P @ error_x
